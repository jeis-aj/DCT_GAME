...
void loop(){
		...
		if (digitalRead( reset_btn ) == 0){
			score = 0;
			return; }
		node_ptr[in		bool isScored  = node_ptr[index].detect_shot();					// update score if shot on target, return true if scored
dex].update();					// update led status (toggle random time)
		if ( isScored ){
